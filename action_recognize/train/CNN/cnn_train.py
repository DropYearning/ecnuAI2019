...
root_path="D:/Datas/Datasets" #原始图片集，五类动作放在五个文件夹中
rate=0.2 #训练集和测试集分割率
data_frame_train = pd.DataFrame()
data_frame_test = pd.DataFrame()
for path in os.listdir(root_path):
    c=get_imlist(root_path+"/"+path)
    tag=path[-1]    #文件夹名字的最后一位是图片的标签
    d=len(c)    # 图像个数
    print(d)
    test_d=(int)(d*rate)
    # 遍历每张图片
    for i in range(d-test_d):
        img = cv2.imread(c[i],cv2.IMREAD_GRAYSCALE)  # 打开图像
        img_ndarray = np.asarray(img, dtype='float64') / 256  # 将图像转化为数组并将像素转化到0-1之间
        data = cv2.resize(img_ndarray,(784,1))
        save1 = pd.DataFrame(data)
        save1[785]=tag
        data_frame_train = data_frame_train.append(save1, ignore_index = True)
    for i in range(d-test_d,d):
        img = cv2.imread(c[i], cv2.IMREAD_GRAYSCALE)  # 打开图像
        img_ndarray = np.asarray(img, dtype='float64') / 256  # 将图像转化为数组并将像素转化到0-1之间
        data = cv2.resize(img_ndarray, (784, 1))
        save2 = pd.DataFrame(data)
        save2[785] = tag
        data_frame_test = data_frame_test.append(save2, ignore_index=True)
#将图片的dataframe保存为csv类型
data_frame_train.to_csv('D:/Datas/Test/train.csv',index=None)
data_frame_test.to_csv('D:/Datas/Test/test.csv',index=None)
#加载图片28×28的csv数据
train_datapath="D:/Datas/Test/train.csv"
test_datapath="D:/Datas/Test/test.csv"
sns.set(style='white', context='notebook', palette='deep')
train = pd.read_csv(train_datapath)
test= pd.read_csv(test_datapath)
X_train = train.drop(labels = ["label"],axis = 1)
Y_train=pd.read_csv(train_datapath,usecols=[784])
X_test = test.drop(labels = ["label"],axis = 1)
Y_test=pd.read_csv(test_datapath,usecols=[784])
'''
查看训练集和测试集的shape
print(X_train.shape)
print(Y_train.shape)
...
plt.show()  #可视化各类的数目
Y_train.apply(pd.value_counts) #统计各类数目
...
# Normalize the data
#X_train = X_train / 255.0 #使用的csv已经标准化过，这步不需要
#X_test = X_test / 255.0
#reshape
